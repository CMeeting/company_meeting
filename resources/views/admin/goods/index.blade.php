                            ...
                            <div class="col-md-4 col-lg-2 col-sm-6 col-xs-12">
                                <div class="form-group">
                                    <select class="form-control"  name="status" tabindex="1">
                                        ...
                                        <option value="2" @if(isset($query)&&$query['status']==1) selected @endif>上架</option>
                                        <option value="1" @if(isset($query)&&$query['status']===0) selected @endif>下架</option>
                                    </select>
                                </div>
                            </div>
                                ...
                                <font class="open_{{$item['id']}}">
                                    @if($item['status'] == 1)
                                        <a data-id="{{$item['id']}}" class="openBtn_{{$item['id']}} abutton cloros1"
                                           data-style="zoom-out" onclick="show({{$item['id']}});">
        ...
        function show(id) {
            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp.code == 0) {
                        if (resp.status == 0) {
                            var htmls = '<a type="button" style="text-decoration: none;color: #f6fff8"   data-id="{$v.id}"  class="openBtn_' + id + ' abutton cloros" data-style="zoom-out" onclick="show(' + id + ');"> <span class="ladda-label">上架</span></a>';
                        } else {
                            var htmls = '<a type="button" style="text-decoration: none;color: #f6fff8"  data-id="{$v.id}"  class="openBtn_' + id + ' abutton cloros1" data-style="zoom-out" onclick="show(' + id + ');"> <span class="ladda-label">下架</span></a>';
                        }
                        $(".open_" + id).html(htmls);
                        layer.close(index);
                    } else {
